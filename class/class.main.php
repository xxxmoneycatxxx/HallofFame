...
//
	...
	function main() {
		...
		echo ("SessionSwitch function start work ...");
		$this->SessionSwitch();
		echo ("Set ID PASS function start work ...");
		$this->Set_ID_PASS();
		ob_start();
		$this->Order();
	...
	function Order() {
		...
		switch(true) {
			case($_GET["menu"] === "auction"):
				...
				$ItemAuction->ItemCheckSuccess();// 競賣終了品物調
				$ItemAuction->UserSaveData();// 競賣品金額各ID配保存
				break;
			...
			switch(true) {
				...
				case($_GET["menu"] === "auction"):
					...
					*/
					...
					$ItemAuction->ItemSaveData();// 變更場合保存。
...
//	敵數返	數～數+2(max:5)
	function EnemyNumber($party) {
		...
		if($min == 5)//5人5匹
			return 5;
		$max	= $min + ENEMY_INCREASE;// 、+2[1人:1～3匹] [2人:2～4匹] [3:3-5] [4:4-5] [5:5]
		if($max>5)
			$max	= 5;
		mt_srand();
...
//	出現確率敵選返
	function SelectMonster($monster) {
		foreach($monster as $val)
			$max	+= $val[0];//確率合計
		$pos	= mt_rand(0,$max);//0～合計 中亂數取
		foreach($monster as $monster_no => $val) {
			$upp	+= $val[0];//時點確率合計
			if($pos <= $upp)//合計低　敵決定
				return $monster_no;
		}
	}
//////////////////////////////////////////////////
...
//	Specify=敵指定(配列)
	function EnemyParty($Amount,$MonsterList,$Specify=false) {
		...
		foreach($MonsterNumbers as $Number) {
			if(1 < $overlap[$Number])//1匹以上出現名前記號。
	...
	function CharStatProcess() {
		...
		switch(true):
			...
			case($_POST["remove"]):
				...
				if(!$char->{$_POST["spot"]}) {// $this  $char 區別注意！
			...
			case($_POST["equip_item"]):
				...
				if(!$this->item["$item_no"]) {//道具所持
					ShowError("Item not exists.","margin15");
					return false;
				}

				$JobData	= LoadJobData($char->job);
				$item	= LoadItemData($item_no);//裝備物
				if( !in_array( $item["type"], $JobData["equip"]) ) {//裝備不可能?
...
print <<< HTML
	...
	<h4>屬性調整 <a href="?manual#statup" target="_blank" class="a0">?</a></h4>
HTML;
...
<?php 
		...
		for($i=0; $i<$char->MaxPatterns(); $i++) {
			...
			foreach($char->skill as $val) {//技option
...
<?php 
	...
	<table>
	<tr><td class="dmg" style="text-align:right">物理攻擊 :</td><td class="dmg"><?php print $char->atk[0]?></td></tr>
	<tr><td class="spdmg" style="text-align:right">魔法攻擊 :</td><td class="spdmg"><?php print $char->atk[1]?></td></tr>
	<tr><td class="recover" style="text-align:right">物理防禦 :</td><td class="recover"><?php print $char->def[0]." + ".$char->def[1]?></td></tr>
	<tr><td class="support" style="text-align:right">魔法防禦 :</td><td class="support"><?php print $char->def[2]." + ".$char->def[3]?></td></tr>
	<tr><td class="charge" style="text-align:right">負重 :</td><td class="charge"><?php print $handle?> / <?php print $char->GetHandle()?></td></tr>
	</table>
...
<?php 
		...
		// 裝備可能物表示 ////////////////////////////////
		if($JobData["equip"])
			$EquipAllow	= array_flip($JobData["equip"]);//裝備可能物(反轉)
		else
			$EquipAllow	= array();//裝備可能物(反轉)
...
<?php 
		...
		// 表示 //////////////////////////////////////
		//include(DATA_SKILL);//ActionPattern移動
...
//	戰。
	function DoppelBattle($party,$turns=10) {
		...
		$battle->SetTeamName($this->name,"");
		$battle->LimitTurns($turns);//最大數10
		$battle->NoResult();
		$battle->Process();//戰鬥開始
		return true;
	}
		...
		switch(true) {
			case($_POST["partjob"]):
				if($this->WasteTime(100)) {
					$this->GetMoney(500);
					ShowResult("工作".MoneyFormat(500)." !(?)","margin15");
					return true;
				} else {
					ShowError("時間無。動。(?)","margin15");
					return false;
				}
			case($_POST["shop_buy"]):
				$ShopList	= ShopList();//賣
				if($_POST["item_no"] && in_array($_POST["item_no"],$ShopList)) {
					...
					$need	= $amount * $item["buy"];//購入必要金
					if($this->TakeMoney($need)) {// 金引判定。
		...
		/*if($_POST["amount"]) {
			...
			if(0 < $amount && $amount < 11) {
				...
				if($this->WasteTime($time)) {
					ShowResult(MoneyFormat($money)." ！","margin15");
					$this->GetMoney($money);
					return true;
				} else {
	...
	function RankProcess(&$Ranking) {
		...
		if($_POST["ChallengeRank"]) {
			...
			*/
			...
			return $Result;// 戰鬥 $Result = "Battle";
		}
		...
		if($_POST["SetRankTeam"]) {
			...
			if(($now - $this->rank_set_time) < RANK_TEAM_SET_TIME) {
				...
				ShowError("離再設定隊伍還需 {$day}日  {$hour}小時 {$min}分 {$sec}秒","margin15");
				return false;
			}
			foreach($this->char as $key => $val) {//
				if($_POST["char_".$key])
...
//	鍛冶屋處理(精鍊)
	function SmithyRefineProcess() {
		...
		// 精鍊始處理
		$this->DeleteItem($_POST["item_no"]);// 道具消變化消
		...
		for($i=0; $i<$times; $i++) {
			...
			if($this->TakeMoney($Price)) {
				$MoneySum	+= $Price;
				$Trys++;
				if(!$obj_item->ItemRefine()) {//精鍊(false=失敗終了)
					break;
				}
			// 金途中場合。
			} else {
...
//	鍛冶屋表示
	function SmithyRefineShow() {
		...
		if($this->item) {
			...
			$goods->ListTableInsert("<tr><td class=\"td9\"></td><td class=\"align-center td9\">精煉費</td><td class=\"align-center td9\">Item</td></tr>"); // 最初最後行表示。
...
//	製作表示
	function SmithyCreateShow() {
		...
		$CreateList->ListTableInsert("<tr><td class=\"td9\"></td><td class=\"align-center td9\">製作費用</td><td class=\"align-center td9\">Item</td></tr>"); // 最初最後行表示。
		...
		foreach($CanCreate as $item_no) {
			...
			if(!HaveNeeds($item,$this->item))// 素材不足次
				continue;
			// NoTable
...
//	道具出品處理
	function AuctionItemExhibitProcess(&$ItemAuction) {
		...
		// 減(所持數多指定場合數調節)
		$_SESSION["AuctionExhibit"]	= time();//2重出品防
		$amount	= $this->DeleteItem($_POST["item_no"],$amount);
		$this->SaveUserItem();

		// 出品
		// $ItemAuction	= new Auction(item);// (2008/2/28:化)
...
//	Union處理
	function UnionProcess() {
		...
		foreach($this->char as $key => $val) {//
			if($_POST["char_".$key]) {
				$MyParty[]	= $this->char[$key];
				$TotalLevel	+= $this->char[$key]->level;//自分PT合計
			}
		}
		// 合計制限
		...
		if($UnionMob["SlaveAmount"])
			$EneNum	= $UnionMob["SlaveAmount"] + 1;//PT同數。
		else
			$EneNum	= 5;// Union含5固定。
...
<?php 
		...
		if($PlaceList["Recruit"])
			print("<li><p><a href=\"?recruit\">招募成員(Recruit)</a></p></li>");
		// 鍛冶屋
		if($PlaceList["Smithy"]) {
			?>
<li>鍛冶屋(Smithy)
...
<input type="submit" class="btn" name="logout" value="登出" style="width:100px">
</form>
<h4>變更隊伍名</h4>
<form action="?setting" method="post">
...
//	戰鬥時選擇記憶
	function MemorizeParty() {
		if($_POST["memory_party"]) {
			...
			//if(5 < count($this->party_memo) )//5人以上馱目
...
//	保存番號變更。
	function SessionSwitch() {
		...
		if($_SESSION):
		//	session_destroy();//Sleipnir...?(最初期)
		//	unset($_SESSION);//大丈夫(馱目)(修正後)
			//結局,foreach1個unset(2007/9/14 再修正)
...
<?php 
	...
	<table><tbody>
	<tr><td colspan="2">帳號和密碼請在4-16位數以內﹝請勿使用中文﹞</td></tr>
	...
	<tr><td></td>
	<td><input type="password" maxlength="16" class="text" name="pass2" style="width:240px">(再次確認)</td></tr>
...
<div style="margin:15px">
<h4>系统訊息</h4>
本服參數：<span class='a0'>100</span>倍經驗、<span class='a0'>100</span>倍金錢<br />
玩家數：<span class='a0'><?php print UserAmount()?></span> / <?php print MAX_USERS?><br />
為了減輕伺服器負載，<?php 
	$Abandon	= ABANDONED;
	print(floor($Abandon/(60*60*24))."天未更新的用戶資料將被刪除！");
print("</div>\n");
	}
...
//	人用人。
	function MyMenu() {
		if($this->name && $this->islogin) { // 人用
			print('<div id="menu">'."\n");
			//print('<span class="divide"></span>');//區切
			print('<a href="'.INDEX.'">個人資料</a><span class="divide"></span>');
			print('<a href="?hunt">出城冒險</a><span class="divide"></span>');
			print('<a href="?item">道具</a><span class="divide"></span>');
			print('<a href="?town">城鎮</a><span class="divide"></span>');
			print('<a href="?setting">設定</a><span class="divide"></span>');
			print('<a href="?log">戰鬥記錄</a><span class="divide"></span>');
			print('<a href="?rank">排行榜</a><span class="divide"></span>');
			if(BBS_OUT)
				print('<a href="'.BBS_OUT.'" target="_balnk">討論區</a><span class="divide"></span>'."\n");
...
<?php 
			print('</div>');
		} else if(!$this->name && $this->islogin) {// 初回人
			print('<div id="menu">');
			...
			print('<a href="?tutorial">基礎教學</a><span class="divide"></span>'."\n");
			print('<a href="?manual">遊戲手冊</a><span class="divide"></span>'."\n");	
			print('<a href="?gamedata=job">遊戲資料</a><span class="divide"></span>'."\n");
			print('<a href="?log">戰鬥記錄</a><span class="divide"></span>'."\n");
			print('<a href="?rank">排行榜</a><span class="divide"></span>'."\n");
			if(BBS_OUT)
