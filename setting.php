<?php
// game setting
define("TITLE","Hall of Fame");//標題
define("MAX_TIME",20000);//最大時間
define("TIME_GAIN_DAY",20000);//每日時間恢復總數
define("MAX_CHAR",10);//最大所持字符数
define("MAX_USERS",500);//最大登録者人数
define("ABANDONED",60*60*24*90);//ゲームを放棄されたとみなす期間
define("CONTROL_PERIOD",60*60*12);//定期管理の周期
define("RECORD_IP",1);//IPを記録するか？(0=NO 1=YES)

// other
define("DEBUG",0);// 0=OFF
define("CHAR_NO_IMAGE","NoImage.gif");// 如果沒有圖片顯示的人物形象
...
define("START_TIME",20000);//ゲーム開始時に持ってるTime
define("START_MONEY",15000);//初期所持金
define("MAX_STATUS",250);//等級最大値
define("GET_STATUS_POINT",5);//升級獲得能力點數
define("GET_SKILL_POINT",2);//升級獲得技能點數
define("MAX_LEVEL",50);//最大レベル
define("SELLING_PRICE",1/5);//売値が設定されていないアイテムの売値→(買値*SELLING_PRICE)
define("REFINE_LIMIT",10);//精錬限界値

define("EXP_RATE",100);//経験値もらえる倍率
define("MONEY_RATE",100);//お金もらえる倍率

define("NEW_NAME_COST",300000);//新しい名前に変更するのに必要なお金
define("BBS_OUT","http://coz.tw/dz6/forum-29-1.html");//外部BBSがあればそのアドレス、無ければ空欄→""
define("BBS_BOTTOM_TOGGLE",1);// 下にあった一行掲示板(0=OFF)
define("AUCTION_TOGGLE",1);// 拍賣場(0=OFF 1=ON)
define("AUCTION_EXHIBIT_TOGGLE",1);// 允許賣方的拍賣(0=OFF 1=ON)
define("JUDGE_LIST_AUTO_LOAD",0);//パターン判断のリストを 1=自動 0=手動追加(若干軽い)
define("AUCTION_MAX",100);//オークション同時に出品できる品数。

// ranking
define("RANK_TEAM_SET_TIME",60*60*48);//ランキングのチーム設定できる周期    小組排名
define("RANK_BATTLE_NEXT_LOSE",60*60*3);//ランキング戦 負けたとき次戦えるまで
...
define("BTL_IMG_TYPE",0);// (0=GD 1=CSS 2=反転済画像使用CSS)
